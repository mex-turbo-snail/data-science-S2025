...
# ## TODO: Pivot the data, rename the columns
  pivot_longer(
    ...
    # names_pattern = "(Estimate|Margin of Error)!!Median income \\(dollars\\)!!(\\d+-person families)"
...
- What *overall* trend do you see between `SE` and population? Why might
  this trend exist?
  - Overall, as population estimates go down the standard error for
    income rises, but at the smae time higher families tend to rise
    more.
- What does this *overall* trend tell you about the relative ease of
  studying small vs large counties?
  - This trend essentially tells us that it is easier to study larger
    counties as it offers less standard error. This can be shown as we
    go down in numbers of population, the chance of getting higher
    numbers in error becomes more likely.
...
## Question: How does income vary in New York City in accordance to population
...
df_data %>%
  ...
  ggplot(aes(x = income_estimate, y = population_estimate)) +
  geom_point() +
  
  geom_point(
    data = . %>% filter(county == "Bronx"),
    color = "red",
    size = 5
  ) +
  geom_point(
    data = . %>% filter(county == "Queens"),
    color = "blue",
    size = 5) +
  geom_point(
    data = . %>% filter(county == "New York"),
    color = "Green",
    size = 5)
```
...
- Here I have a scatter plot showing the relationship between the
  population estimate and income estimates in New York (where I am
  original from). All counties are included but, I highlighted the
  Bronx, Queens and Manhattan (Which is often refereed to as New York).
  Initially, I found interesting was that even though Queens has the
  higher population out of the 3, Manhattan still has the higher income.
  After some thought, this made sense because more people commute to
  work in Manhattan and choose to live in other boroughs like Queens or
  The Bronx because of the price of living in Manhattan (Due to the
  limited estate). Overall I thought this was interesting to look at and
  wonder what other things may be affecting this.
